// =====================================================
// EDGE FUNCTION: process-exam
// Processa documentos de exames usando Google Gemini
// =====================================================
// SECRETS NECESSÁRIOS:
// - GOOGLE_AI_API_KEY (ou use a key hardcoded para testes)
// - RESEND_API_KEY (para envio de alertas)
// =====================================================
...
serve(async (req) => {
  ...
  try {
    ...
    // IMPORTANTE: Substitua estas URLs pelas do seu projeto Supabase
...
3. Look for the date of the exam (data de coleta or data de cadastro). Use YYYY-MM-DD format.`;
    ...
    if (parsedData.results && parsedData.results.length > 0) {
      ...
      if (examResults.length === 0) {
        ...
        return new Response(
          JSON.stringify({ success: true, message: 'Exam processed but no valid results found', resultsCount: 0 }),
      ...
      // Send email alerts for warning/danger results
      ...
      if (alertResults.length > 0) {
        ...
        try {
          const alertResponse = await fetch(
            `${supabaseUrl}/functions/v1/send-exam-alerts`,
            {
              method: 'POST',
              headers: {
